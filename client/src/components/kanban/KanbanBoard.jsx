...
import { getColumns, createColumn } from '../../api/columnApi';
...
export default function KanbanBoard() {
  ...
  useEffect(() => {
    ...
    const loadBoardColumns = async () => {
      try {
        ...
        let boardColumns = await getColumns(boardId);

        /*
         * Make sure every board has:
         * To Do
         * Doing
         * Done
         *
         * Missing columns are created individually.
         */
        for (const title of DEFAULT_COLUMNS) {
          const exists = boardColumns.some(
            (column) =>
              normalizeColumnTitle(column.title) ===
              normalizeColumnTitle(title)
          );
  ...
  const orderedColumns = useMemo(() => {
    ...
    return [...columns].sort((a, b) => {
      const aOrder =
        order[normalizeColumnTitle(a.title)] ?? 99;

      const bOrder =
        order[normalizeColumnTitle(b.title)] ?? 99;
            ...
            return (
              ...
              >
                <div className="kanban-column-header">
                  ...
                  <button
                    type="button"
                    className="icon-btn kanban-add-task-btn"
